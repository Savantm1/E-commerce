...
const BlogPage = (props) => {
  ...
  const middleBlogItems = props.blogArr.map((element, index) => {
    if (index >= 2) {
      return <BlogMidItem key={index} postImg={postImg} data={element} />;
    }
    return null;
  });
  ...
  return (
    <>
      <div className="blog_page">
        <div className="blog_page__main_row">{mainRowBlogItems}</div>
        <div className="blog_page__section">
          ...
          <div className="blog_page_items_block">{middleBlogItems}</div>
        </div>
        {/* <PaginationSection /> */}
      </div>
    </>
